...
#' library(data.table)
#' dt_pep <- fread("data/Yogurtexample_QR188-205.csv", integer64 = "double")
...
processPeptides <- function(peptides_file,
  ...
  build_nested_list <- function(meta, grp_cols) {
    ...
    if (length(grp_cols) > 1) { #more levels need to go
      ...
      for (v in vals) {
        ...
        dt.ttest <- dt.mean # first save a copy for potential t.test analysis
  ...
  dt.peptides.int.ttest <- flatten_nested(nested.peptides, grp_cols, "ttest")
