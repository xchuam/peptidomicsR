...
in-memory `data.frame`/`data.table`. When you pass a path, the function reads
the file with `data.table::fread()`, so tab-, comma-, or semicolon-delimited
text exports (including the default MaxQuant `.txt` peptide export) can be used
directly. If your data lives in Excel workbooks, import them first (e.g.
`openxlsx::read.xlsx()`) and pass the resulting table.

#### Required input files

##### 1. MaxQuant peptide export (`peptides_file`)

-   Must contain the structural columns used to derive cleavage information and
    peptide identities.
-   All intensity columns referenced in the metadata file must be present and
    **numeric** so that replicate means can be computed.
...
-   Each row maps a single intensity column to the replicate and all associated
    grouping variables (e.g. digest stage, treatment).
-   The non-`Intensity.column` fields must **uniquely identify** the row; they
    define the nested grouping structure used to calculate replicate and group
    summaries.
-   It is recommended to use dots (`.`) instead of spaces in column names for
    easier programming.
...
-   A curated default mapping ships with the package as an in-memory dataset you can load
    via `data("peptidomics_protein_mapping_example")`.
...
*Ma, X., & Ren, Q. (2025). peptidomicsR (Version 1.0.0-alpha) [Computer software]*
