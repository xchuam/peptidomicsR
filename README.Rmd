...
`peptidomicsR` provides functions to process, filter, analyze, and visualize peptidomics data, especially from MaxQuant protein digestion studies. This README introduces the package through four parts:
...
Start with a minimal call (sequence is auto-looked-up if present):

```{r}
plot_pep_align(result, protein_name = "P02662")
```

If the sequence is missing in the mapping, supply it manually using the packaged
example table:
...
Add grouping filters, replicate mode, and axis controls together:
...
If your peptide table uses non-standard column names, override them explicitly:
...
-   A curated default mapping ships with the package as an in-memory dataset you can load via `data("peptidomics_protein_mapping_example")`.
...
*Ma, X., & Ren, Q. (2025). peptidomicsR (Version 1.1.0-alpha) [Computer software]*
